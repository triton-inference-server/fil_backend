...
//#endif
  ...
  std::cout << "HERRING:  \n";
  for (auto i=0; i < std::min(output.size(), std::size_t{10}); ++i) {
    std::cout << output[i] << ", ";
    output[i] = 0;
  }
  std::cout << "\n";
  auto start_index = std::size_t(std::max(0, int(output.size()) - 10));
  for (auto i=start_index; i < std::min(output.size(), start_index + std::size_t{10}); ++i) {
    std::cout << output[i] << ", ";
    output[i] = 0;
  }
  std::cout << "\n";
