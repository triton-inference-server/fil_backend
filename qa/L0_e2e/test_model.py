...
def run_classification_model(client, model_name, model_repo,model_format, X, instance_kind, num_class,num_features, use_experimental_optimizations,expected_class, expected_proba, expected_shap_sum):
    # probability output
    if expected_proba is not None:
        ...
        np.testing.assert_allclose(result['output__0'], expected_proba, rtol=1e-3, atol=1e-3)
    ...
    if instance_kind == "KIND_GPU" and expected_shap_sum is not None:
        ...
        result = predict(client, model_name, X)
        np.testing.assert_allclose(result['treeshap_output'].sum(axis=-1), expected_shap_sum, rtol=1e-3, atol=1e-3)

def run_regression_model(client, model_name, model_repo,model_format, X, instance_kind, num_features, use_experimental_optimizations,expected):
    ...
    if instance_kind == "KIND_GPU":
        ...
        result = predict(client, model_name, X)
        np.testing.assert_allclose(result['treeshap_output'].sum(axis=-1), expected, rtol=1e-3, atol=1e-3)
...
def get_xgb_classifier(num_features, num_class):
    ...
    return model.fit(data_with_categoricals(1000,num_features, 91), rng.randint(0, num_class, 1000))
...
def test_xgb_classification_model(client, model_repo, instance_kind, num_class, use_experimental_optimizations):
    ...
    run_classification_model(client, base_name, model_repo,"xgboost", X, instance_kind,num_class, num_features, use_experimental_optimizations, model.predict(X), model.predict_proba(X), model.predict(X, output_margin=True))
    
@pytest.mark.parametrize("use_experimental_optimizations", [True, False])
@pytest.mark.parametrize("instance_kind", available_instance_types)
@pytest.mark.parametrize("num_class", [2, 4])
def test_xgb_classification_model_json(client, model_repo, instance_kind, num_class, use_experimental_optimizations):
    num_features = 50
    model = get_xgb_classifier(num_features, num_class)
    ...
    X = data_with_categoricals(100,num_features,13)
    run_classification_model(client, base_name, model_repo,"xgboost_json", X, instance_kind,num_class, num_features, use_experimental_optimizations, model.predict(X), model.predict_proba(X), model.predict(X, output_margin=True))
...
def test_xgb_regression_model(client, model_repo, instance_kind, use_experimental_optimizations):
    ...
    run_regression_model(client, base_name, model_repo,"xgboost", X, instance_kind,num_features, use_experimental_optimizations, model.predict(X))
...
def get_lgbm_classifier(num_features, num_class):
    ...
    return model.fit(data_with_categoricals(1000,num_features,291), rng.randint(0, num_class, 1000))
...
def test_lgbm_classification_model(client, model_repo, instance_kind, num_class, use_experimental_optimizations):
    ...
    X = data_with_categoricals(100,num_features,13)
    run_classification_model(client, base_name, model_repo,"lightgbm", X, instance_kind,num_class, num_features, use_experimental_optimizations, model.predict(X), model.predict_proba(X), model.predict(X, raw_score=True))
...
def test_lgbm_regression_model(client, model_repo, instance_kind, use_experimental_optimizations):
    ...
    run_regression_model(client, base_name, model_repo,"lightgbm", X, instance_kind,num_features, use_experimental_optimizations, model.predict(X))

@lru_cache
def get_sklearn_classifier(num_features, num_class):
    rng = np.random.RandomState(1132)
...
@pytest.mark.parametrize("instance_kind", available_instance_types)
@pytest.mark.parametrize("num_class", [2, 5])
def test_sklearn_classification_model(client, model_repo, instance_kind, num_class, use_experimental_optimizations):
    num_features = 50
    model = get_sklearn_classifier(num_features, num_class)
    base_name = "sklearn{}_class_{}".format(num_class, instance_kind)
    model_dir = get_model_directory(model_repo, base_name)
    save_sklearn_as_tl(model_dir, model)
    rng = np.random.RandomState(1133)
    X = rng.random((100,num_features)).astype(np.float32)
    expected_shap_sum = model.predict_proba(X) if num_class > 2 else model.predict_proba(X)[:,1]
    run_classification_model(client, base_name,model_repo, "treelite_checkpoint", X, instance_kind, num_class, num_features, use_experimental_optimizations, model.predict(X), model.predict_proba(X), expected_shap_sum)

@lru_cache
def get_sklearn_regressor(num_features):
...
@pytest.mark.parametrize("use_experimental_optimizations", [True, False])
@pytest.mark.parametrize("instance_kind", available_instance_types)
def test_sklearn_regression_model(client, model_repo, instance_kind, use_experimental_optimizations):
    num_features = 50
    model = get_sklearn_regressor(num_features)
    base_name = "sklearn_regressor"
    model_dir = get_model_directory(model_repo, base_name)
    save_sklearn_as_tl(model_dir, model)
...
@pytest.mark.parametrize("instance_kind", available_instance_types)
@pytest.mark.parametrize("num_class", [2, 5])
def test_cuml_classification_model(client, model_repo, instance_kind, num_class, use_experimental_optimizations):
    num_features = 50
