 ...
 * for each request
 */
std::vector<TRITONBACKEND_Input*>
get_backend_inputs(
    std::string& input_name, std::vector<TRITONBACKEND_Request*>& requests)
{
 ...
 */
std::pair<std::vector<std::vector<int64_t>>, std::vector<uint32_t>>
get_input_shapes(
  ...
  for (auto& input : inputs) {
    ...
    if (input_shapes.size() != 0 &&
        (input_shapes[0].size() != cur_dims_count ||
         !std::equal(
             input_shapes[0].begin() + 1, input_shapes[0].end(),
             cur_shape + 1))) {
      throw TritonException(
          TRITONSERVER_errorcode_enum::TRITONSERVER_ERROR_INTERNAL,
...
std::vector<TRITONBACKEND_Output*>
get_backend_outputs(
    std::string& name, std::vector<TRITONBACKEND_Response*>& responses,
    ...
    const std::vector<std::vector<int64_t>>& shapes)
{
  ...
  for (std::size_t r = 0; r < responses.size(); ++r) {
    ...
    triton_check(TRITONBACKEND_ResponseOutput(
        responses[r], &output, name.c_str(), dtype, shapes[r].data(),
        shapes[r].size()));
...
std::vector<RawOutputBuffer>
get_raw_output_buffers(
    std::vector<TRITONBACKEND_Output*> all_outputs,
    TRITONSERVER_MemoryType& output_memory_type,
    const std::vector<std::vector<int64_t>>& shapes, int64_t device_id)
{
...
struct InputBatch {
  ...
  InputBatch(
      ...
      std::pair<std::size_t, std::size_t>&& extent,
      std::vector<std::vector<int64_t>>&& shapes)
      : data(std::move(data)), extent(extent), shapes(shapes)
  {
  }
};
...
)
{
  ...
  auto raw_buffers = get_raw_output_buffers<TritonDtype<T>::value>(
      backend_outputs, output_memory_type, tensor_shapes,
