...
std::enable_if_t<D == kayak::device_type::gpu && kayak::GPU_ENABLED, void> predict(
  ...
  auto algorithm_selector = (
    (kayak::raw_index_t{categorical} << 2) +
    (kayak::raw_index_t{lookup} << 1) +
  ...
  auto tree_count = padded_size(forest.tree_count(), WARP_SIZE);
  // We want as many threads per block as possible without going over the
  // maximum allowed shared memory per block. Each thread uses an additional
  // number of memory entries equal to the number of classes times the number
  // of rows per block.
  auto rows_per_block = padded_size(in.rows(), blocks) / blocks;
  auto threads_per_block = downpadded_size(
    max_shared_mem_entries_per_block / 
    (num_class * rows_per_block),
    WARP_SIZE
  );
  threads_per_block = std::max(std::min(
    threads_per_block, get_max_threads_per_block(device_id)
  ), WARP_SIZE);
