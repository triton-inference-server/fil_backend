...
struct simple_tree {
  using node_type =
      simple_node<value_t, feature_index_t, offset_t, output_index_t>;
  ...
  template <
      bool missing_values_in_row, bool categorical_model,
  ...
  auto evaluate_tree_node(std::size_t node_index, float const* row) const
  {
    ...
    if constexpr (categorical_model) {
      if (!has_categorical_nodes) {
        result = evaluate_tree_node_<
            missing_values_in_row, false, inclusive_threshold>(node_index, row);
      } else {
        result = evaluate_tree_node_<
            missing_values_in_row, true, inclusive_threshold>(node_index, row);
      }
    } else {
      result = evaluate_tree_node_<
          missing_values_in_row, false, inclusive_threshold>(node_index, row);
 ...
 private:
  template <
      bool missing_values_in_row, bool categorical_tree,
      bool inclusive_threshold>
      ...
      if (present) {
        if constexpr (categorical_tree) {
          if (!categorical_node[node_index]) {
            result =
                evaluate_node<false, inclusive_threshold>(node, feature_value);
          } else {
            result =
                evaluate_node<true, inclusive_threshold>(node, feature_value);
          }
        } else {
          result =
              evaluate_node<false, inclusive_threshold>(node, feature_value);
...
struct lookup_tree {
  ...
  template <
      ...
      std::enable_if_t<
          is_container_specialization<tree_output_type, std::vector>::value,
          bool> = true>
