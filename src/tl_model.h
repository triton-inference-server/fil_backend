...
struct TreeliteModel {
  TreeliteModel(std::filesystem::path const& model_file,
                SerializationFormat format,
                std::shared_ptr<treelite_config> tl_config)
    : handle_{load_tl_handle(model_file, format)},
      num_classes_{tl_get_num_classes(handle_)},
  ...
  {
    if (handle_ != nullptr) { TreeliteFreeModel(handle_); }
  }
               ...
               bool predict_proba) const
  {
    ...
    if (gtil_output_size != output.size()) {
      output_buffer = rapids::Buffer<float>{gtil_output_size, rapids::HostMemory};
    }

    // Actually perform inference
    auto out_result_size = std::size_t{};
    auto gtil_result     = TreeliteGTILPredict(
      handle_, input.data(), samples, output_buffer.data(), 1, &out_result_size);
    if (gtil_result != 0) {
      throw rapids::TritonException(rapids::Error::Internal, TreeliteGetLastError());
    }

    // Copy back to expected output location
    if (gtil_output_size != output.size()) {
      rapids::copy<float, float>(output, output_buffer, std::size_t{}, output.size());
    }
    ...
    if (num_classes_ == 1 && predict_proba) {
      ...
      while (i > 0) {
        --i;
        output.data()[i] = ((i % 2) == 1) ? output.data()[i / 2] : 1.0f - output.data()[i / 2];
      }
    } else if (num_classes_ == 1 && !predict_proba && tl_config_->output_class) {
      std::transform(
        output.data(), output.data() + output.size(), output.data(), [this](float raw_pred) {
